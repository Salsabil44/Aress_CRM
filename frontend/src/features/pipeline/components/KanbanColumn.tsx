  ...
  return (
    <div
      className={`flex flex-col min-w-[260px] max-w-[280px] rounded-xl transition-all duration-150 ${
        isDragOver
          ? 'bg-primary-50/40 ring-1 ring-primary-200 ring-inset'
          : 'bg-slate-50/50'
      }`}
    ...
    >
      <div className="px-3 py-3 flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-[12px] text-slate-600">{status}</h3>
        </div>
        <span className="text-[10px] font-semibold text-slate-400 bg-white px-1.5 py-0.5 rounded border border-slate-100 tabular-nums">
          {leads.length}
        </span>
      </div>
      <div className="flex-1 px-2 pb-2 space-y-2 overflow-y-auto max-h-[calc(100vh-280px)] min-h-[80px]">
        {leads.length === 0 && (
          <div className={`flex items-center justify-center h-16 border border-dashed rounded-lg transition-colors duration-150 ${
            isDragOver ? 'border-primary-300 bg-primary-50/20' : 'border-slate-200'
          }`}>
            <p className="text-[11px] text-slate-300">Drop leads here</p>
          </div>
        )}
        {leads.map((lead) => (
