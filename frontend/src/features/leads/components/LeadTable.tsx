      ...
      <div className="bg-white dark:bg-slate-800 rounded-xl border border-slate-200/60 dark:border-slate-700 overflow-hidden">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-900/50">
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Lead</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Contact</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Source</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Status</th>
                {isManager && (
                  <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Owner</th>
                )}
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Created</th>
                <th className="text-left px-5 py-3 text-[11px] font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider">Actions</th>
              </tr>
            </thead>
            <tbody>
              {filtered.length === 0 ? (
                <tr>
                  <td colSpan={isManager ? 7 : 6} className="px-5 py-16 text-center">
                    <div className="flex flex-col items-center">
                      ...
                      <p className="text-[13px] text-slate-400 dark:text-slate-500 font-medium">No leads found</p>
                      <p className="text-[11px] text-slate-300 dark:text-slate-600 mt-0.5">Try adjusting your search or filters</p>
                    </div>
                  </td>
                </tr>
              ) : (
                filtered.map(lead => (
                  <LeadRow key={lead.id} lead={lead} onEdit={onEdit} onDelete={onDelete} isManager={isManager} />
                ))
              )}
            </tbody>
          </table>
        ...
        <div className="px-5 py-2.5 border-t border-slate-50 dark:border-slate-700 bg-slate-50/30 dark:bg-slate-900/30">
          <p className="text-[11px] text-slate-400 dark:text-slate-500">
            Showing <span className="font-medium text-slate-600 dark:text-slate-400">{filtered.length}</span> of {leads.length} leads
          </p>
        </div>
      </div>
    </div>
