import { Pencil, Trash2, Building2, Mail, Phone, User } from 'lucide-react';
...
export function LeadRow({ lead, onEdit, onDelete, isManager }: LeadRowProps) {
  const { user } = useAuth();
    ...
    <tr className="group border-b border-slate-50 dark:border-slate-700 last:border-0 hover:bg-slate-50/50 dark:hover:bg-slate-700/30 transition-colors duration-100">
      <td className="px-5 py-3.5">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="font-medium text-slate-800 dark:text-slate-200 text-[13px] leading-tight">{lead.name}</p>
            <div className="flex items-center gap-1 mt-0.5">
              <Building2 className="w-3 h-3 text-slate-300 dark:text-slate-600" />
              <span className="text-[11px] text-slate-400 dark:text-slate-500">{lead.company}</span>
            </div>
          </div>
        </div>
      </td>
      <td className="px-5 py-3.5">
        <div className="space-y-0.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[12px] text-slate-500 dark:text-slate-400">{lead.email}</span>
          </div>
          {lead.phone && (
            <div className="flex items-center gap-1.5">
              <Phone className="w-3 h-3 text-slate-300 dark:text-slate-600" />
              <span className="text-[11px] text-slate-400 dark:text-slate-500">{lead.phone}</span>
            </div>
          )}
        </div>
      </td>
      <td className="px-5 py-3.5">
        <span className="text-[11px] text-slate-500 dark:text-slate-400 bg-slate-50 dark:bg-slate-700 px-2 py-1 rounded-md font-medium">{lead.source}</span>
      </td>
      <td className="px-5 py-3.5">
      ...
      {isManager && (
        <td className="px-5 py-3.5">
          <div className="flex items-center gap-1.5">
            ...
            <span className={`text-[11px] font-medium ${isOwnLead ? 'text-primary-600 dark:text-primary-400' : 'text-slate-500 dark:text-slate-400'}`}>
